...
from utils.graph_util import display_graph
from utils.sophy_util import chat, company_qna_chat
...
with open("resources\company.md", "r") as f:
...
class AgentResponse(BaseModel):
    ...
    pickedRoute: str = Field(
        description="The route to proceed in, can only be: 'continue_chat', 'crisis_helpline', 'company_qa'"
    )
...
def sophy_chat(chat_state: SophyState):
    ...
    exchange, conv_sum, conversation_history, latest_exchanges, user_input = chat(
        llm,
        exchange,
        conv_sum,
...
def agent_edges(
    chat_state: SophyState,
) -> Literal["Sophy", "crisisHandler", "companyQA"]:
...
def company_qa(chat_state: SophyState, retriever=retriever):
    ...
    exchange, conversation_history = company_qna_chat(
        llm, retriever, conversation_history, user_input, exchange
    )
